...
# Index
* [Run cucumber/gherkin-syntaxed specs with Cypress.io](#cypress-io)
* [Test Automation for MS](#automation-ms)
...
```
<a name="automation-ms"></a>

## Test Automation for MS


## Overview

This test automation model is structured under the automation/ folder, that contains all the code necessary to the automated tests execution. The folder is now inside the project root folder.

The automation/ folder contains many subfolders, the use of each one will be explained below.

![](folder_tree.png)

### test-pod/

This folder contains the code, the feature files and utilities for all the automated tests.

#### test-pod/cypress/integration/

This folder contains all the feature files. They are pulled from a separate repository.

#### test-pod/cypress/support/pages/

This folder contains a file for each relevant page of the application. Each page contains mappings of the locators on that page and the fuctions that interact with them (for example, click on a button or fill a field).

#### test-pod/cypress/support/step_definitions/

This folder contains the main code, the methods that implement the user actions declared on the feature files. These methods call the methods from the page files.
The methods are split in 3 files: given, when and then, one file for each type of cucumber expression. This makes the code more organized and the reuse of cucumber expressions between different feature files easier.

#### test-pod/cypress/support/utils

This folder is not strictly necessary, it can contain any other utilities that the developer deems necessary.

#### test-pod/cypress/reports

This folder is meant to contain the reports generated by the execution (for example, Allure reports).

## Important Notes

### Different package.json files

The automation/ folder is in fact a module because it has its own package.json file (the test execution won't refer to the main application package.json file anymore). This change was made in order to avoid library conflicts and to make the reuse easier.
A consequence of this is the need to run a "yarn install" command inside the automation module to install all the dependencies.
